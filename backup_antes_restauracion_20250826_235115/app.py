...
def generar_voucher(operacion_id, tamaño):
    ...
    if not current_user.es_admin and operacion.usuario_id != current_user.id:
        flash('No tienes permisos para ver esta operación', 'error')
...
@app.route('/voucher-preview')
def voucher_preview():
    """Muestra una vista previa de cómo se verían los vouchers"""
    return render_template('voucher_preview.html')

@app.route('/seleccionar-voucher/<int:operacion_id>')
@login_required
def seleccionar_voucher(operacion_id):
    """Página para seleccionar el tamaño del voucher"""
    operacion = Operacion.query.get_or_404(operacion_id)
    return render_template('seleccionar_voucher.html', operacion=operacion)
...
def generar_voucher(operacion_id, tamaño):
    ...
    if tamaño == '58mm':
        ...
        return redirect(url_for('seleccionar_voucher', operacion_id=operacion_id))
...
def registrar_operacion():
    if request.method == 'POST':
        ...
        try:
            ...
            # Obtener el ID de la operación recién creada
            operacion_id = db.session.execute(text("SELECT LASTVAL()")).scalar()
...
def api_reportes_operaciones():
    try:
        ...
        for op in operaciones:
            ...
            usuario_nombre = op.usuario.nombre_completo if op.usuario else 'Usuario no encontrado'
    ...
    try:
        ...
        with app.app_context():
            ...
            if not admin_exists:
                ...
                return "✅ Base de datos ya inicializada", 200
