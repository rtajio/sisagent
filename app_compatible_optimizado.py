...
- ELIMINAR SIN REPREGUNTAR: Si el usuario dice "elimina la operación", "borra esa", "quita la última" — SIN especificar ID — ASUME que habla de la ÚLTIMA operación registrada por ese usuario en ESA SUCURSAL. Llama a `buscar_operaciones()` sin filtro para obtener la más reciente, luego elimínala directamente con `proponer_eliminar_operacion(operacion_id=<última>)`. NO preguntes cuál eliminar.
