#!/usr/bin/env python3
"""
Script para arreglar todo el sistema de una vez
"""
...
def arreglar_todo():
    ...
    print("🔧 ARREGLANDO TODO EL SISTEMA")
    print("=" * 40)
    
    # 1. Eliminar archivos de base de datos
    ...
    with app.app_context():
        ...
        # Guardar sucursales primero
        db.session.commit()
        print("   ✅ Sucursales guardadas")
...
def probar_sistema():
    ...
    with app.app_context():
        # Verificar que el admin puede acceder a sucursales
        admin = Usuario.query.filter_by(username='admin').first()
        if admin and admin.es_admin:
            sucursales = Sucursal.query.filter_by(activa=True).all()
            print(f"✅ Admin puede acceder a {len(sucursales)} sucursales")
            
            # Simular la función registrar_operacion
            if len(sucursales) > 0:
                print("✅ Función registrar_operacion funcionará correctamente")
                return True
            else:
                print("❌ No hay sucursales disponibles")
                return False
        else:
            print("❌ Admin no configurado correctamente")
            return False

if __name__ == "__main__":
    try:
        # Arreglar todo
        success = arreglar_todo()
        
        if success:
            # Probar el sistema
            test_ok = probar_sistema()
            
            if test_ok:
                print(f"\n🎉 SISTEMA ARREGLADO Y FUNCIONANDO")
                ...
                print(f"6. Intenta registrar una operación")
            else:
